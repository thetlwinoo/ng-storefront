...
export class BoxShortcutsComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        if ( this._cookieService.check('BOX2.shortcuts') )
        {
            this.shortcutItems = JSON.parse(this._cookieService.get('BOX2.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle): void
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this._cookieService.set('BOX2.shortcuts', JSON.stringify(this.shortcutItems));
